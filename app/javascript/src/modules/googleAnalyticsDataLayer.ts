...
const getCookiePreferences = (): string => Cookies.get('cookie_preferences_tailspend') ?? '{}'

const getCookiePreferencesSaved = (): string => Cookies.get('cookie_preferences_tailspend_saved') ?? '{}'

const setCookiePreferencesSaved = (cookiePreferences: CookiePreferences) => {
  Cookies.set('cookie_preferences_tailspend_saved', JSON.stringify(cookiePreferences), { expires: 365 })
}
